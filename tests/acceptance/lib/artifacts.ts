...
export type EventDirection = 'runner-to-cli' | 'cli-to-runner';
...
export class Artifacts {
  ...
  appendEvent(scenario: string, direction: EventDirection, message: unknown): void {
  ...
  appendScenarioStderr(scenario: string, value: string): void {
    const filename = `scenario-${scenario.replace(/[^a-z0-9_-]/gi, '_')}.stderr.log`;
