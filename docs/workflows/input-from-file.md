...
- **If using a heredoc**, make sure the closing `EOF` is on its own line with no leading spaces or tabs. Even one space before `EOF` will cause the shell to miss it:
  ...
  ```
- **Make sure you wrote `--input -`** (with the dash after a space), not just `--input`. The `-` tells MainWP Control to read from stdin.
...
This shows you exactly which fields are required, which are optional, and what types they expect (string, number, array, etc.). Use this output to build your JSON parameter file.
